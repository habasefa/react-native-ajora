...
describe("AjoraCore error handling", () => {
  describe("agent error events", () => {
    it("emits AGENT_RUN_ERROR_EVENT when agent sends RunError event", async () => {
      ...
      expect(
        errors.some((e) => e.code === AjoraCoreErrorCode.AGENT_RUN_ERROR_EVENT)
      ).toBe(true);
      const evt = errors.find(
        (e) => e.code === AjoraCoreErrorCode.AGENT_RUN_ERROR_EVENT
      )!;
    ...
    it("emits AGENT_RUN_FAILED_EVENT when agent triggers onRunFailed", async () => {
      ...
      expect(
        errors.some((e) => e.code === AjoraCoreErrorCode.AGENT_RUN_FAILED_EVENT)
      ).toBe(true);
      const evt = errors.find(
        (e) => e.code === AjoraCoreErrorCode.AGENT_RUN_FAILED_EVENT
      )!;
  ...
  describe("http errors", () => {
    ...
    it("emits AGENT_RUN_FAILED when proxied runtime agent run fails (REST)", async () => {
      ...
      expect(
        errors.some((e) => e.code === AjoraCoreErrorCode.AGENT_RUN_FAILED)
      ).toBe(true);
      const evt = errors.find(
        (e) => e.code === AjoraCoreErrorCode.AGENT_RUN_FAILED
      )!;
  ...
  describe("internal processing errors (tools)", () => {
    it("emits TOOL_ARGUMENT_PARSE_FAILED then AGENT_RUN_FAILED when tool args JSON is invalid", async () => {
      ...
      expect(
        errors.some(
          (e) => e.code === AjoraCoreErrorCode.TOOL_ARGUMENT_PARSE_FAILED
        )
      ).toBe(true);
    ...
    it("emits TOOL_HANDLER_FAILED and continues run when tool handler throws", async () => {
      ...
      expect(
        errors.some((e) => e.code === AjoraCoreErrorCode.TOOL_HANDLER_FAILED)
      ).toBe(true);
