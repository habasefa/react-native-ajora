...
describe("StateManager - Basic State Tracking", () => {
  ...
  it("should track state when run starts", async () => {
    ...
    const storedState = ajoraCore.getStateByRun("agent1", "thread1", runId);
  ...
  it("should return undefined for non-existent run", () => {
    const storedState = ajoraCore.getStateByRun(
      "agent1",
      "thread1",
      "non-existent-run"
    );
...
describe("StateManager - Multiple Runs", () => {
  ...
  it("should track multiple sequential runs independently", async () => {
    ...
    expect(ajoraCore.getStateByRun("agent1", "thread1", "run1")).toEqual(
      run1State
    );
    expect(ajoraCore.getStateByRun("agent1", "thread1", "run2")).toEqual(
      run2State
    );
    expect(ajoraCore.getStateByRun("agent1", "thread1", "run3")).toEqual(
      run3State
    );
  });
  ...
  it("should list all run IDs for a thread", async () => {
    ...
    const runIds = ajoraCore.getRunIdsForThread("agent1", "thread1");
  ...
  it("should return empty array for thread with no runs", () => {
    const runIds = ajoraCore.getRunIdsForThread("agent1", "thread-no-runs");
  ...
  it("should handle state updates in the middle of a run", async () => {
    ...
    expect(ajoraCore.getStateByRun("agent1", "thread1", runId)).toMatchObject({
      count: 1,
      status: "processing",
    });
...
describe("StateManager - Message Tracking", () => {
  ...
  it("should associate new messages with runs", async () => {
    ...
    const associatedRunId = ajoraCore.getRunIdForMessage(
      "agent1",
      "thread1",
      "msg1"
    );
  ...
  it("should associate messages from snapshot with runs", async () => {
    ...
    expect(ajoraCore.getRunIdForMessage("agent1", "thread1", "msg1")).toBe(
      runId
    );
    expect(ajoraCore.getRunIdForMessage("agent1", "thread1", "msg2")).toBe(
      runId
    );
    expect(ajoraCore.getRunIdForMessage("agent1", "thread1", "msg3")).toBe(
      runId
    );
  });
  ...
  it("should return undefined for non-existent message", () => {
    const runId = ajoraCore.getRunIdForMessage(
      "agent1",
      "thread1",
      "non-existent-msg"
    );
...
describe("StateManager - Multiple Agents and Threads", () => {
  ...
  it("should track state for multiple agents independently", async () => {
    ...
    expect(ajoraCore.getStateByRun("agent2", "thread2", "run1")).toEqual(
      agent2State
    );
  });
  ...
  it("should track messages for multiple agents independently", async () => {
    ...
    expect(ajoraCore.getRunIdForMessage("agent2", "thread2", "msg2")).toBe(
      "run1"
    );
    // Cross-agent lookups should return undefined
    expect(
      ajoraCore.getRunIdForMessage("agent1", "thread1", "msg2")
    ).toBeUndefined();
    expect(
      ajoraCore.getRunIdForMessage("agent2", "thread2", "msg1")
    ).toBeUndefined();
  });
  ...
  it("should handle same agent with multiple threads", async () => {
    ...
    expect(ajoraCore.getStateByRun("agent3", "thread-a", "run1")).toEqual(
      threadAState
    );
    expect(
      ajoraCore.getStateByRun("agent3-threadb", "thread-b", "run1")
...
describe("StateManager - State Isolation", () => {
  ...
  it("should deep copy state to prevent external mutations", async () => {
    ...
    const retrievedState = ajoraCore.getStateByRun(
      "agent1",
      "thread1",
      runId
    ) as any;
    ...
    const retrievedAgain = ajoraCore.getStateByRun(
      "agent1",
      "thread1",
      runId
    ) as any;
...
describe("StateManager - Edge Cases", () => {
  ...
  it("should handle agent without agentId gracefully", async () => {
    ...
    const state = ajoraCore.getStateByRun("test", "thread1", "run1");
  ...
  it("should handle empty state object", async () => {
    ...
    const storedState = ajoraCore.getStateByRun("agent1", "thread1", "run1");
  ...
  it("should handle messages without input parameter", async () => {
    ...
    const runId = ajoraCore.getRunIdForMessage("agent1", "thread1", "msg1");
    expect(runId).toBeUndefined();
  });
});
...
describe("StateManager - Real-world Scenarios", () => {
  ...
  it("should track a complete conversation flow", async () => {
    ...
    expect(
      ajoraCore.getRunIdForMessage("chatbot", "user-session-123", msg1.id)
    ).toBe(run1Id);
    expect(
      ajoraCore.getRunIdForMessage("chatbot", "user-session-123", msg2.id)
    ).toBe(run1Id);
    expect(
      ajoraCore.getRunIdForMessage("chatbot", "user-session-123", msg3.id)
    ).toBe(run2Id);
    expect(
      ajoraCore.getRunIdForMessage("chatbot", "user-session-123", msg4.id)
    ).toBe(run2Id);

    // Verify states are tracked correctly
    const run1State = ajoraCore.getStateByRun(
      "chatbot",
      "user-session-123",
      run1Id
    );
    ...
    const run2State = ajoraCore.getStateByRun(
      "chatbot",
      "user-session-123",
      run2Id
    );
    ...
    const runIds = ajoraCore.getRunIdsForThread("chatbot", "user-session-123");
  ...
  it("should handle tool execution flow with state changes", async () => {
    ...
    expect(
      ajoraCore.getRunIdForMessage("chatbot", "user-session-123", userMsg.id)
    ).toBe(runId);
    expect(
      ajoraCore.getRunIdForMessage(
        "chatbot",
        "user-session-123",
        toolCallMsg.id
      )
    ...
    // Verify final state
    const finalState = ajoraCore.getStateByRun(
      "chatbot",
      "user-session-123",
      runId
    );
