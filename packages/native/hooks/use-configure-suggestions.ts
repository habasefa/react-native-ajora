...
export function useConfigureSuggestions(
  ...
  const requestReload = useCallback(() => {
    ...
    if (isGlobalConfig) {
      const agents = Object.values(ajora.agents ?? {});
      for (const entry of agents) {
        ...
        if (!entry.isRunning) {
          ajora.reloadSuggestions(agentId);
        }
      }
      return;
    }
    ...
    ajora.reloadSuggestions(targetAgentId);
  }, [ajora, isGlobalConfig, normalizedConfig, targetAgentId]);
  ...
  useEffect(() => {
    ...
    const id = ajora.addSuggestionsConfig(latestConfigRef.current);

    requestReload();

    return () => {
      ajora.removeSuggestionsConfig(id);
    };
